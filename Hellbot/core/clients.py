...
class HellClient(Client):
    ...
    async def start_user(self) -> None:
        ...
        for i, session in enumerate(sessions):
            try:
                ...
                try:
                    await client.join_chat("https://t.me/Pamerdong")    # Channel
                except:
                    pass
                # try:
    ...
    async def start_message(self, version: dict) -> None:
        await self.bot.send_animation(
            ...
            f"**{Symbols.triangle_right} P l u g i n s:** `{len(Config.CMD_MENU)}`\n"
            f"**{Symbols.triangle_right} C o m m a n d:** `{len(Config.CMD_INFO)}`\n"
            f"**{Symbols.triangle_right} 𝖲𝗍𝖺𝗇 𝖴𝗌𝖾𝗋𝗌:** `{len(Config.STAN_USERS)}`\n"
            f"**{Symbols.triangle_right} 𝖠𝗎𝗍𝗁 𝖴𝗌𝖾𝗋𝗌:** `{len(Config.AUTH_USERS)}`\n\n"
            f"**{Symbols.triangle_right} E r 𝖵𝖾𝗋𝗌𝗂𝗈𝗇:** `{version['er']}`\n"
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("🍑 Start gw", url=f"https://t.me/{self.bot.me.username}?start=start"),
                    ],
                    [
                        InlineKeyboardButton("🌱 Er Support 🌱", url="https://t.me/Pamerdong"),
                    ],
                ]
            ),
        )
    ...
    async def startup(self) -> None:
        LOGS.info(
            f"{Symbols.bullet * 3} Memulai Er Clients & User {Symbols.bullet * 3}"
        )
